...
@Composable
fun EmployeeCreateLoanScreenContainer(
    navController: NavHostController,
    entry: NavBackStackEntry,
    modifier: Modifier = Modifier
){
    val viewModel: EmployeeLoanViewModel = viewModel(
        viewModelStoreOwner = entry,
        factory = EmployeeLoanViewModelFactory(
            employeeLoanRepository = LoanRepositoryImpl(RetrofitInstance.loanApiService)
        )
    )
    ...
    LaunchedEffect(state) {
        ...
        state.createdLoan?.loanId?.let {
            navController.navigateAndClear(
                ...
                popUpToRoute = AppDestinations.EmployeeCreateLoanScreen.route
            )
        }
    }
}